...
func TestReplaceFirewallRules(t *testing.T) {
	cases := []struct {
		name     string
		request  *ReplaceFirewallRulesRequest
		response string
		expected *ReplaceFirewallRulesResponse
		err      bool
	}{
		{
			name: "success",
		...
		t.Run(c.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if r.Method != http.MethodPut {
					t.Errorf("Expected method %q, got %q", http.MethodPut, r.Method)
				}
			...
			client := New("test-api-key", WithBaseUrl(server.URL))
			res, err := client.ReplaceFirewallRules(context.Background(), c.request)
